...
public interface UsersRepository extends JpaRepository<Users, Long> {
	...
	@Query(value = "SELECT u.* FROM users u " + "WHERE u.active_flag = :activeFlag "
			+ "AND (u.name ILIKE CONCAT('%', :keyword, '%') " + "OR u.email ILIKE CONCAT('%', :keyword, '%') "
			+ "OR u.role ILIKE CONCAT('%', :keyword, '%')) "
			+ "ORDER BY u.created_time DESC", countQuery = "SELECT count(*) FROM users u "
					+ "WHERE u.active_flag = :activeFlag " + "AND (u.name ILIKE CONCAT('%', :keyword, '%') "
					+ "OR u.email ILIKE CONCAT('%', :keyword, '%') "
					+ "OR u.role ILIKE CONCAT('%', :keyword, '%'))", nativeQuery = true)
	Page<Users> findAllBySearch(@Param("keyword") String keyword, @Param("activeFlag") String activeFlag,
			Pageable pageable);
